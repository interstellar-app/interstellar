...
cat lemmy_instances.txt piefed_instances.txt mbin_instances.txt mastodon_instances.txt | sort | uniq -i > instances.txt
...
$(awk '{ print "  \047"$0"\047: ServerSoftware.mbin," }' mastodon_instances.txt)
};
EOF
...
rm instances.txt lemmy_instances.txt piefed_instances.txt mbin_instances.txt mastodon_instances.txt
